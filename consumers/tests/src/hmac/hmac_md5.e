...
feature
	...
	finish
		local
			...
			hmac_hash: MD5
		do
			create hash_inner.make_filled (0, 16)
			message_hash.do_final (hash_inner, 0)
			create hmac_hash.make
			hmac_hash.sink_special_lsb (opad, 0, 63)
			hmac_hash.sink_special_lsb (hash_inner, 0, 15)
			create hash_outer.make_filled (0, 16)
			hmac_hash.do_final (hash_outer, 0)
			create hmac.make_from_bytes (hash_outer, 0, 15)
			finished := True
		ensure
			finished
		end
