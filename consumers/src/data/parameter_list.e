	...
	as_form_url_encoded_string : READABLE_STRING_GENERAL
		...
		do
			if parameters.count = 0 then
				...
				across parameters as elem loop
...
feature -- Change Elements
	add_query_string (query_string : READABLE_STRING_GENERAL)
		...
		do
			...
			across query_string.split (param_separator.at (1)) as elem
			loop
				...
				parameters.force (create {PARAMETER}.make(l_key.as_string_32, l_value.as_string_32))
			end
		end


feature -- Status Report
	has (a_param : PARAMETER) : BOOLEAN
		do
...
feature -- Sort
	sort : PARAMETER_LIST
		local
			...
			l_sort :QUICK_SORTER[PARAMETER]
			l_comp : COMPARABLE_COMPARATOR [PARAMETER]
		do
